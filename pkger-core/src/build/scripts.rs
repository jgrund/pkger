...
macro_rules! run_script {
    ($phase:literal, $script:expr, $dir:expr,  $ctx:ident) => {{
        ...
        if let Some(dir) = &$script.working_dir {
            ...
            trace!("working directory: {}", _dir.display());
            opts = opts.working_dir(&_dir);
        } else {
            trace!("using default working directory: {}", $dir.display());
        ...
        for cmd in &$script.steps {
            trace!("processing command '{}'", cmd.cmd);
            if let Some(images) = &cmd.images {
                trace!("only execute on: {:?}", images);
                if !images.contains(&$ctx.build.target.image().to_owned()) {
                    trace!("'{}' not found in images", $ctx.build.target.image());
                    if !cmd.has_target_specified() {
                        debug!("skipping, excluded by image filter");
                        continue;
                    }
                }
            }
            ...
            if !cmd.should_run_on($ctx.build.target.build_target()) {
                debug!("skipping, shouldn't run on target");
                continue;
            }

            debug!("running command '{}'", cmd.cmd);
